...
void arch_virt_map_page(uint64_t *pml4, void *vaddr, uint64_t paddr, uint64_t flags) {
  ...
  if (!(pml4[pml4_idx] & PAGE_PRESENT)) {
    ...
    pml4[pml4_idx] = pdpt_phys | PAGE_PRESENT | PAGE_RW | PAGE_USER; // Allow user for flexibility
  }
...
void arch_virt_map(void) {
  ...
  // Map all usable physical memory (HHDM and Identity)
  for (uint64_t i = 0; i < kernel.mem.map.count; i++) {
    memreg_t *region = &kernel.mem.map.regions[i];

    // Only skip memory that is physically broken or reserved by hardware/firmware
    ...
    // Alignment fix: If a region size is not page aligned (e.g., 0x1500),
    // we must map the full upper page to avoid cutting off data
    ...
    for (uint64_t offset = 0; offset < size; offset += PAGE_SIZE) {
      ...
      arch_virt_map_page(pml4, (void *)virt_hhdm, phys, PAGE_PRESENT | PAGE_RW);
    }
  }
  ...
  for (uint64_t offset = 0; offset < kernel_size; offset += PAGE_SIZE) {
    // Calculate offset from the start of the kernel image

    uint64_t curr_virt = kernel_start_v + offset;
    uint64_t curr_phys = kernel_phys_base + (curr_virt - kernel_virt_base);

    // Map it
    arch_virt_map_page(pml4, (void*)curr_virt, curr_phys, PAGE_PRESENT | PAGE_RW);
  }
